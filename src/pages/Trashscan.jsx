...
function TrashScan() {
  ...
  useEffect(() => {
    const loadModel = async () => {
      try {
        const graphModel = await tf.loadGraphModel('/tfjs_model_fix/model.json');
        setModel(graphModel);
  ...
  return (
    <div className="text-center p-6">
      <h1 className="text-3xl font-bold mb-4">TrashScan: mengetahui jenis sampahmu hanya dalam hitungan detik.</h1>
      <p>Teknologi klasifikasi berbasis AI, membantu kamu membedakan jenis sampah organik, anorganik, dan B3 dengan cepat dan akurat.</p>

      {/* Mode switch */}
      <div className="mb-4">
        <button
          onClick={() => setMode('webcam')}
          className={`px-4 py-2 mr-2 rounded ${mode === 'webcam' ? 'bg-black text-white' : 'bg-gray-200'}`}
        >
          Webcam
        </button>
        <button
          onClick={() => setMode('upload')}
          className={`px-4 py-2 rounded ${mode === 'upload' ? 'bg-black text-white' : 'bg-gray-200'}`}
        >
          Upload
        </button>
      </div>
      ...
      {mode === 'webcam' ? (
        <video ref={videoRef} width="700" height="350" autoPlay muted className="rounded-lg border shadow" />
      ) : (
        <div className="flex flex-col items-center">
          <label className="mb-2 font-medium text-gray-700">Pilih gambar sampah untuk diklasifikasikan: </label>
          <input
            type="file"
            accept="image/*"
